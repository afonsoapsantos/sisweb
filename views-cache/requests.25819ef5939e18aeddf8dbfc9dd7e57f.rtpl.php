        ...
        <tbody align='center'>
            <?php $counter1=-1;  if( isset($requests) && ( is_array($requests) || $requests instanceof Traversable ) && sizeof($requests) ) foreach( $requests as $key1 => $value1 ){ $counter1++; ?>
            	<tr>
            		...
            		<td>
            			<a href="/manager/requests/generate/order/<?php echo htmlspecialchars( $value1["idrequest"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" class="circular ui black icon button">
            				<i class="clipboard icon"></i>
            			</a>
		                <a href="/manager/requests/<?php echo htmlspecialchars( $value1["idrequest"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" class="circular ui green icon button">
		                  <i class="edit icon"></i>
		                </a>
		                <a href="/manager/requests/<?php echo htmlspecialchars( $value1["idrequest"], ENT_COMPAT, 'UTF-8', FALSE ); ?>/delete" onclick="return confirm('Deseja realmente excluir?')" class="circular ui red icon button">
