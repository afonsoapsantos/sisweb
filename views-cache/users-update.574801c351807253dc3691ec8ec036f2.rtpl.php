<?php if(!class_exists('Rain\Tpl')){exit;}?><title>Sisweb - Usuários</title>
	<div class="ui segment">
		...
		<form class="ui form" action="/admin/users/<?php echo htmlspecialchars( $user["pkuser"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" method="POST">
				...
				<div class="field" align="left">
	                ...
	                <select class="uis selection" name='fkstatususer'>
	                	...
	                	<?php $counter1=-1;  if( isset($status) && ( is_array($status) || $status instanceof Traversable ) && sizeof($status) ) foreach( $status as $key1 => $value1 ){ $counter1++; ?>
	                		<option class="item" value="<?php echo htmlspecialchars( $value1["pkstatus"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" 
	                		<?php if( $user["fkstatususer"] == $value1["pkstatus"] ){ ?>selected<?php } ?>><?php echo htmlspecialchars( $value1["txnamestatus"], ENT_COMPAT, 'UTF-8', FALSE ); ?></option>
	                	<?php } ?>
	                </select></br>
	                ...
	                <select class="ui selection" name="fkusertype">
	                    ...
	                    <option value="<?php echo htmlspecialchars( $value1["idusertype"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" <?php if( $user["fkusertype"] == $value1["idusertype"] ){ ?>selected<?php } ?>><?php echo htmlspecialchars( $value1["txnameusertype"], ENT_COMPAT, 'UTF-8', FALSE ); ?></option>
	                    <?php } ?>
	                </select></br>
				</div>
				<div class="ui buttons">
